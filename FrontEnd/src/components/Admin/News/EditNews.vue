<template lang="">
    <div class="container">
        ...
        <div class="content">
            <form class="formulario" @submit.prevent="editNews">
...
<script setup>
    ...
    const newId = defineProps({
        id: [String, Number]
    })

    async function editNews(){
        try{
            await http.put(`/news/update/${newId.id}`,{
    ...
    async function showNews(){
        try{
            const {data} = await http.get(`/news/show/${newId.id}`);
            news.value.title = data.title;
            news.value.description = data.description;
        }catch(error){
            console.error("Erro ao receber noticia " + error);
        }
    }
